...
function Sprite(scene, imageFile, width, height){ 
  ...
  this.checkBounds = function(){
    ...
    if (this.boundAction==OTHERTHING)
    {
      if (offRight){
	this.x = leftBorder;
      } // end if
	...
	this.x = rightBorder;
      ...
      if (this.y < topBorder+120){
	this.y = topBorder+120;
      }
    }
...
function Scene(){
    ...
    this.canvas.style.backgroundImage="url('betternebula.png')";
    document.getElementById("center").appendChild(this.canvas);
    this.context = this.canvas.getContext("2d");
	...
	document.onkeyup = this.clearKeys;
      ...
      this.intID = setInterval(localUpdate, 35);
...
WRAP = 0; BOUNCE = 1; STOP = 3; DIE = 4; CONTINUE = 5; OTHERTHING=6;